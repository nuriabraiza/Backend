...
const chatdb = new Chat(configSqlite);
...
socket.on("new-message", async (data) => {
  let listaMensajes = await chatdb.getAllMsj();
  const nuevoMensaje = {
  ...
  await chatdb.addMsj(nuevoMensaje);

  io.sockets.emit("messages", await chatdb.getAllMsj());
});
